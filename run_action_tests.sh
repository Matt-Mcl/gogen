...
while [ $retry_count -lt $max_retries ]; do
    python3 manage.py test gogen.tests.test_views --parallel=4 --noinput && python3 manage.py test gogen.tests.test_generated gogen.tests.test_hints --noinput && python3 manage.py test gogen.tests.test_database --noinput && python3 manage.py test gogen.tests.test_models --noinput
    if [ $? = 0 ]; then
        exit 0
    else
